...
static inline bool is_vowel (char ltr) {
    return strchr("aeiou", ltr) != nullptr;
};

// Для добавления символа в автосортирующуюся строку
void ordered_insert (const  char chr, std::string &str) {
    size_t i = 0;
    // c_str() кончается '\0'
...
combination_map get_combinations(const char *line, size_t min_combination_l);
...
std::string filter_common_pairs(const char *line, const pair_list &pairs);
...
int main(void) {
    ...
    combination_map combinations = get_combinations(line.c_str(), MIN_VOWEL_COMBO_LEN);
    pair_list most_common_pairs = get_most_common_pairs(combinations, MOST_COMMON_PAIRS_LEN);
    std::string result = filter_common_pairs(line.c_str(), most_common_pairs);
...
combination_map get_combinations(const char *it, size_t min_combination_l) {
    ...
    for (; *it != '\0'; ++it) {
        if (*it == ' ') {
            ...
            unsigned char lowered = (unsigned char ) std::tolower(*it);
            if (is_vowel(lowered)) ordered_insert(lowered, combination);
        }
    }
...
std::string filter_common_pairs(const char* it, const pair_list &pairs) {
    std::string result;
    std::string combination;
    // ws -- начало слова
    auto ws = it;
