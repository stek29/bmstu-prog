...
void Tree::walk(std::function<void(tree_element_t&)> func, bool reverse) {
    if (head != nullptr) {
        ...
        head->traverse([&func](tnode_t *node){func(node->element);}, order);
    }
}

tnode_t* Tree::create_node(const tree_element_t &el) {
    return new tnode_t(el);
}
...
void Tree::insert(const tree_element_t &el) {
    tnode_t *leaf = create_node(el);
    if (head != nullptr) {
        head->append(leaf);
    } else {
        head = leaf;
